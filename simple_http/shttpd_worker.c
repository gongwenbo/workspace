	...
	for(;SCHEDULESTATUS== STATUS_RUNNING;)
	{
		...
		switch(retval)
		{
			...
			case 0:/*timeout*/
				...
				break;
			default:
				if(FD_ISSET(ss, &rfds))/*check the file*/
				{
					int sc = accept(ss, (struct sockaddr*)&client, &len);
					printf("client comming\n");
					i = WORKER_ISSTATUS(WORKER_IDEL);/*find the thread isnot in work*/
					if(i == -1)
					{
						i =  WORKER_ISSTATUS(WORKER_DETACHED);
						if(i != -1)/*if there thread is on work,create a new one*/
							Worker_Add( i);
					}
					if(i != -1)/*if the thread is not on work*/
					{
						wctls[i].conn.cs = sc;
