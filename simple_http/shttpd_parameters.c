		...
		switch (c)
		{
			case 'c': 	/*CGI root arg*/
				...
				if(l_opt_arg && l_opt_arg[0]!=':'){
					...
					memcpy(conf_para.CGIRoot, l_opt_arg, len +1);/*renew it*/
				}
				
				break;
			case 'd': 	/*default file name*/
				l_opt_arg = optarg;
				if(l_opt_arg && l_opt_arg[0]!=':'){
					len = strlen(l_opt_arg);
				...
				break;
			case 'f': 	/*config file name and arg*/
				l_opt_arg = optarg;
				if(l_opt_arg && l_opt_arg[0]!=':'){
					len = strlen(l_opt_arg);
